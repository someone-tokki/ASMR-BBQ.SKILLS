...
- Completed:
  - Added `scripts/scan_audio_scope.py` to list audio folders/files for user scope confirmation before model calls.
  - Added `docs/preflight_confirmation.md` with the fixed user-facing confirmation template and run profile flow.
  - Added `scripts/prepare_run_profile.py` and `scripts/check_preflight.py`; model calls can now be gated on confirmed `run_profile.json`.
  - Extended `scripts/manage_model_profile.py` with `sync-run-profile`.
  - Updated `scripts/batch_translate_srt_omlx.py` with `safe`/`fast`/`turbo` presets, dynamic default settings, optional `--workers`, `--profile`, and `--plan-only`.
  - Updated `scripts/translate_srt_omlx.py` with dynamic `fast` defaults, presets, chunk-local signature cache under `<output_stem>.translate_chunks/`, manifest status records, and optional translation profile output.
  - Added `scripts/prepare_asr_audio_cache.py` for conservative ASR prepared-cache/segment-plan records with optional ffmpeg normalization.
  - Updated `SKILL.md`, `docs/task_routing.md`, both workflow docs, and `docs/user_guide.md` to reflect mandatory Preflight, audio-folder scope confirmation, mode/preset mappings, translation cache/profile behavior, and ASR prepared cache.
- Modified files:
  - `SKILL.md`
  - `docs/preflight_confirmation.md`
  - `docs/task_routing.md`
  ...
  - `docs/user_guide.md`
  - `docs/implementation_log.md`
  - `scripts/scan_audio_scope.py`
  - `scripts/prepare_run_profile.py`
  - `scripts/check_preflight.py`
  - `scripts/manage_model_profile.py`
  - `scripts/batch_translate_srt_omlx.py`
  - `scripts/translate_srt_omlx.py`
  - `scripts/prepare_asr_audio_cache.py`
- Validation commands:
  - `python -B -m py_compile scripts/*.py`
  - `git diff --check`
  - `python scripts/scan_audio_scope.py /private/tmp/asmr_preflight_fixture/RJ000000 --json-out .../audio_scope_report.json`
  - `python scripts/prepare_run_profile.py ... --scope selected_dirs --confirmed --overwrite`
  - `python scripts/check_preflight.py ... --stage all --json`
  - `python scripts/manage_model_profile.py sync-run-profile ...`
  - `python scripts/prepare_asr_audio_cache.py ... --recursive --json-out .../asr_prepared/report.json`
  - `python scripts/translate_srt_omlx.py ... --preset fast --plan-only`
  - `python scripts/batch_translate_srt_omlx.py ... --preset fast --plan-only --profile .../batch_profile.json`
  - negative Preflight fixture with `--scope ask`
  - `python scripts/check_environment.py --dry-run-install --skip-api`
- Validation results:
  - Syntax checks passed.
  - `git diff --check` passed.
  - Audio scope fixture listed two audio folders and wrote JSON.
  - Confirmed run profile passed `check_preflight.py`; `scope=ask` correctly failed the hard gate.
  - Model profile successfully synced from confirmed run profile.
  - ASR prepared cache wrote per-track `segments.json` and a report without applying VAD.
  - Single-file and batch translation plan-only commands generated manifests without model calls.
  - `check_environment.py --dry-run-install --skip-api` remains WARN with no FAIL; warnings are non-blocking local environment notes.
- Open questions:
  - Real model throughput should be profiled on an actual ASMR run before raising default `--workers` above 1.
