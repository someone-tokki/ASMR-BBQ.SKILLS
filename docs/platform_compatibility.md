...
- If a registered MCP exposes suitable subtitle translation/QC tools, it may be selected explicitly as backend `mcp`. It is not part of `auto`: the agent must present the server/tool in preflight, confirm the route and provider/model when known, then call its subtitle tool rather than handling a provider API key itself.
...
- Stage model choices are binding for workflow steps. Translation, mandatory QC, and optional QC refinement must call the resolved local/configured `/chat/completions` endpoint through the workflow scripts, or use the explicitly selected MCP subtitle tool. If the configured route is missing, it is a setup/backend problem; it is not permission for the agent to use its own model as a hidden replacement QC model.
- Before a direct `/chat/completions` translation, mandatory QC, or additional QC refinement, run `scripts/prepare_model_stage.py "$PROJECT_ROOT" <translate|qc> --previous-stage <stage> --from-config` to verify the target chat model can answer now. For a selected MCP route, verify the exact expected MCP subtitle tool is visible instead; do not probe a fabricated base URL. This check is separate from Preflight: Preflight records intended model choices, while stage preparation/tool discovery checks the current live route.
...
8. Never silently switch models after subtitles have already been generated for a project; record the backend/model change in the project notes or `model_profile.json`. At each stage boundary, confirm the target route and model: ASR uses `/audio/transcriptions` with a Whisper-class model; direct translation/QC uses `/chat/completions` with the configured chat model, while an MCP route uses the user-confirmed subtitle tool. Use `prepare_model_stage.py` before direct chat model stages; for MCP, verify tool availability. Second and later QC refinement rounds follow the same rule.
