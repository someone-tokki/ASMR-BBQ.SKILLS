...
- Use `${ASMR_BBQ_HOME:-~/ASMR-BBQ}/providers.json` for shared user-level chat provider profiles when cloud APIs or shared local endpoints are used. Register them with `scripts/manage_provider_registry.py`; store provider type, base URL, default model, API-key environment variable name, and supported stages only. Do not store plaintext API keys in provider registry, project config, model profile, manifests, reports, or logs.
- Translation and QC support `openai-compatible` and `anthropic` providers. OpenAI-compatible providers use `/chat/completions`; Anthropic uses the native Messages API and must not receive OpenAI-only fields such as `chat_template_kwargs`.
