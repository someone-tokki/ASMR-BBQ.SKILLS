...
- 每条路线都必须在交付前完成结构校验、风险扫描、ASMR 可读性检查、第一轮强制模型 QC、QC 闭环校验和学习库更新，除非用户明确只要求一个只读检查、格式转换，或在 Preflight 中选择 `draft` 粗烤模式。QC 候选必须逐项 `accept/reject/defer`；证据不足的高风险项写入 `$PROJECT_ROOT/qc_manual_review.md/json`，可以带标记交付但不得静默忽略。学习库更新时要先解析学习路径，再把项目记录写入 `$PROJECT_ROOT/learning/work_record.md`；收尾时若用户选择整理学习库，可复用候选先进入 shared corpus review，只有明确 approve 的条目才迁移到用户长期学习库。
