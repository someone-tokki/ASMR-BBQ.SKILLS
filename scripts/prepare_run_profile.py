...
def questionnaire_record(path_value: str, project_root: Path) -> dict[str, str]:
    path = Path(path_value)
    if not path.is_file():
        raise SystemExit(f"Mandatory preflight questionnaire not found: {path}")
    content = path.read_text(encoding="utf-8")
    if PREFLIGHT_MARKER not in content:
        raise SystemExit("Preflight questionnaire is not a generated ASMR-PREFLIGHT v1 checklist.")
    resolved = path.resolve()
    try:
        stored_path = resolved.relative_to(project_root.resolve()).as_posix()
    except ValueError:
        stored_path = resolved.as_posix()
    return {"path": stored_path, "sha256": hashlib.sha256(content.encode("utf-8")).hexdigest()}
...
def missing_fields(profile: dict[str, Any]) -> list[str]:
    ...
    if profile.get("confirmed"):
        ...
        if not isinstance(questionnaire, dict) or not questionnaire.get("path") or not questionnaire.get("sha256"):
            missing.append("preflight_questionnaire")
...
def build_profile(args: argparse.Namespace, existing: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    profile: dict[str, Any] = {
        ...
        "preflight_questionnaire": questionnaire_record(args.preflight_questionnaire, Path(args.project_root)) if args.preflight_questionnaire else {},
...
def main() -> int:
    ...
    parser.add_argument("--preflight-questionnaire", default="", help="Generated preflight_questionnaire.md shown to the user before confirmation.")
    parser.add_argument(
        "--confirmed-item",
        action="append",
        default=[],
    ...
    if args.confirmed and not args.preflight_questionnaire:
        raise SystemExit("--confirmed requires --preflight-questionnaire generated by render_preflight_questionnaire.py.")
