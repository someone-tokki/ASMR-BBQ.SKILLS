...
2. Identify audio directories, script availability, existing `.ja.asr.srt`, existing `.zh.srt`/`.zh.vtt`, promo/trial/DLC/EX/bonus audio, desired output format, and the route from `docs/task_routing.md`.
   ...
   - Before new ASR, scan audio variants with `scripts/select_asr_audio_source.py`. User-selected audio always wins. Otherwise, use a no-SE track by default only when its normalized track name and duration both match regular audio within the report's 2% tolerance and `requires_review=false`. If a matched no-SE track has both MP3 and WAV candidates, prefer MP3 for faster ASR unless warnings suggest a cropped/preview/placeholder file; keep the final subtitle names mapped to the original work tracks. After scope selection, run `scripts/resolve_wav_only_asr_tracks.py`; if any selected ASR input is WAV-only, ask whether only those tracks should use a temporary stereo MP3 cache or original WAV.
...
8. When this run needs new ASR, resolve the selected scope with `scripts/resolve_wav_only_asr_tracks.py`. If it reports WAV-only ASR tracks, list them and confirm either `mp3_cache` or `original_wav`; the temporary MP3 option must preserve stereo and applies only to those tracks.
9. Write the confirmed choices to `$PROJECT_ROOT/run_profile.json` with `scripts/prepare_run_profile.py`, including `--confirmation-source`, `--confirmation-text`, `--preflight-questions-presented`, and either `--audio-scope-report` or `--audio-scope-summary`.
