...
- Treat `$PROJECT_ROOT/model_profile.json` as the user-editable stage model preference file. It may specify separate backend/provider/provider_profile/base_url/model values for ASR, translation, and QC. It is not a secrets file; never store API keys in it.
- Treat `${ASMR_BBQ_HOME:-~/ASMR-BBQ}/providers.json` as the shared user-level provider registry for cloud/local chat endpoints. It may store provider type, base URL, default model, API-key environment variable name, and supported stages, but never plaintext API keys. If the user gives an API key to the agent, use it only for the current run/probe or ask the user to set the named environment variable; do not write it to project files, registry files, logs, manifests, or reports.
...
Cloud/API provider rule: for cloud or shared local chat endpoints, first register the endpoint with `scripts/manage_provider_registry.py add --name <profile> --provider <openai-compatible|anthropic> --base-url <url-or-host:port> --model <model> --api-key-env <ENV> --stage translate --stage qc`, then bind project stages with `scripts/manage_model_profile.py set-stage "$PROJECT_ROOT" translate --provider-profile <profile> --model <model>` and/or `qc`. The provider registry stores only connection metadata and the environment variable name. Translation/QC scripts may use `--provider-profile <profile>` and will resolve provider/base URL/model/API key at runtime.
