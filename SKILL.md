...
2. Identify audio directories, script availability, existing `.ja.asr.srt`, existing `.zh.srt`/`.zh.vtt`, promo/trial/DLC/EX/bonus audio, desired output format, and the route from `docs/task_routing.md`.
   ...
   - Before new ASR, scan audio variants with `scripts/select_asr_audio_source.py`. User-selected audio always wins. Otherwise, use a no-SE track by default only when its normalized track name and duration both match regular audio within the report's 2% tolerance and `requires_review=false`. If a matched no-SE track has both MP3 and WAV candidates, prefer MP3 for faster ASR unless warnings suggest a cropped/preview/placeholder file; keep the final subtitle names mapped to the original work tracks.
