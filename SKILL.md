...
- Before ASR or translation, scan the source work folder for all target audio folders with `scripts/scan_audio_scope.py`. Include the discovered folders in the single consolidated preflight questionnaire after all read-only discovery is complete; do not ask scope as a standalone early question. Do not skip audio merely because its filename lacks a main-story number, has a promo/trial/DLC label, or appears outside the main audio folder. Process all discovered audio only when the user confirms `all`; otherwise honor the selected folders/files recorded in `run_profile.json`.
- Treat macOS AppleDouble files whose basename starts with `._` as metadata, never as audio, subtitles, or candidate inputs. Scanners, ASR source selection, WAV-only resolution, cache preparation, and ASR runners must ignore them; never ask the user to decide how to process one.
...
2. Identify audio directories, script availability, existing `.ja.asr.srt`, existing `.zh.srt`/`.zh.vtt`, promo/trial/DLC/EX/bonus audio, desired output format, and the route from `docs/task_routing.md`.
   - Run `scripts/scan_audio_scope.py "$SOURCE_PROJECT_DIR" --json-out "$PROJECT_ROOT/audio_scope_report.json"`, but do not ask for scope yet. Keep the folder list for the single consolidated preflight questionnaire after all read-only scans finish. Translation scope is not confirmed until it is written to `$PROJECT_ROOT/run_profile.json`.
   ...
   - Before new ASR, scan audio variants with `scripts/select_asr_audio_source.py`. Translation scope and ASR input are separate decisions: selecting a WAV folder for subtitle coverage does not select WAV as the ASR source. User-selected ASR audio always wins. Otherwise, use a no-SE track by default only when its normalized track name and duration both match regular audio within the report's 2% tolerance and `requires_review=false`. For every remaining selected WAV track, `scripts/resolve_wav_only_asr_tracks.py` must first look across all work directories for a same-track, duration-matched, non-preview native MP3. If `ffprobe` is unavailable, it may use an exact normalized track-name match only when the MP3 is neither preview-like nor suspiciously small, and must label that fallback in the report. Use the selected MP3 automatically and keep final subtitle names mapped to the selected work track. Ask about WAV conversion only when no such safe native MP3 exists; never ask merely because the selected delivery folder is WAV.
3. Default translation and QC model calls to an existing local OpenAI-compatible service port:
   ...
   - If any registered MCP exposes an appropriate subtitle translation/QC tool and the user selects it, use that tool instead of exposing a cloud key or reproducing its prompts in the agent. This is an explicit invocation route, not a replacement for preflight or for recording the selected provider/model.
...
6. Use `--translate-backend auto` unless the user explicitly chooses `ollama`, `omlx`, `lmstudio`, `mcp`, or another backend. Offer `mcp` only when a suitable tool is actually available to the current agent.
7. Run environment detection before ASR, translation, or QC. This is part of the read-only discovery phase and must finish before asking the user any preflight question:
...
ASR optimization rule: improve ASR cautiously and only when the selected backend/tool supports the option. Prefer user-selected audio first; otherwise prefer no-SE audio only when normalized track name and duration mapping are clear. If no-SE MP3 and WAV versions are both available for the same matched track, use MP3 first for speed, but fall back to WAV or ask for review when the MP3 looks cropped, preview-only, unusually tiny, or otherwise suspicious. After the user’s consolidated preflight reply confirms scope and WAV preference, and only when new ASR is needed, run `scripts/resolve_wav_only_asr_tracks.py`. Its report is authoritative: if a safe native MP3 exists for a track, temporary WAV-to-MP3 conversion is prohibited. `mp3_cache` is permitted only when `wav_only_choice_required=true`; call `scripts/prepare_asr_audio_cache.py --normalize --normalize-format mp3 --wav-only-report "$PROJECT_ROOT/wav_only_asr_report.json"` so the script rejects every non-approved input. Ask again only when the user did not provide a WAV preference or their answer conflicts with the resolved scope. Record the choice and tracks in `run_profile.json`. With `mp3_cache`, cache output must preserve two channels. After ASR completes, run `--cleanup-mp3` to delete generated MP3 intermediates while retaining plans and reports. VAD may skip long silence or pure ambience only when supported and explicitly chosen, but it must not remove low-volume whispers, breaths, important pauses, or quiet dialogue. When splitting long audio, use overlap/stride to avoid boundary truncation, pass the previous segment transcript as prompt/context when supported, and record segment-level progress so interrupted runs can resume without discarding completed segments. Keep audio-file-level resume regardless. For ASMR breaths, ear-licking, kissing, and repeated sound effects, it is acceptable for final subtitles to simplify repeated noise; do not encourage ASR or later translation to generate meaningless repeated text walls.
...
**Conversation gate:** do all read-only discovery first: project resolution, audio scope scan, audio-source scan, existing-subtitle check, and environment detection. Then send exactly one structured, blocking preflight questionnaire before any model call. Do not ask about scope early and then ask models later; do not emit a separate WAV question after the user responds. The one questionnaire must explicitly ask for ASR, translation, and QC choices as separate items; never compress them into “use the recommended models?” or infer one stage’s model from another. It must also include the user’s conditional WAV preference (`mp3_cache` or `original_wav`) so that, after scope resolution, the agent can apply it without another round trip. Do not create a confirmed profile, call a production command, or continue after a partial answer: keep the unresolved items visible and ask only for those items. A user saying “use defaults” authorizes defaults only when it clearly covers all still-unresolved choices.

**Hard execution gate:** after read-only scans, generate the canonical confirmation sheet with `scripts/render_preflight_questionnaire.py` and present that generated file verbatim. `prepare_run_profile.py --confirmed` requires this file and records its digest; production ASR, translation, and QC commands require `--project-root` and reject missing, changed, or incomplete confirmation records. There is no normal-production bypass flag.

**Every-task question set:** the single questionnaire must cover: (1) translation scope, (2) quality mode, (3) ASR backend/base URL/model and reuse versus new ASR, (4) translation backend/base URL/model, (5) QC backend/base URL/model, (6) output format, and (7) conditional WAV preference. When a suitable MCP tool is available, present `mcp` as a translation/QC route alongside direct backends; identify the server/tool and its configured provider/model only when that information is available. State the scan findings next to each proposal, including discovered folders, existing ASR, reachable backends and MCP tools, and native MP3 candidates. WAV preparation is a conditional choice, but never a later follow-up: ask the user to choose a standing preference for any truly WAV-only selected tracks; if the selected scope later resolves to a safe native MP3 or reused ASR, explicitly record that the preference was not needed. Defaults may be proposed, but none of these decisions may be silently assumed.

The agent must:

1. Complete `scan_audio_scope.py`, `select_asr_audio_source.py`, existing-subtitle discovery, and `check_environment.py` in read-only mode.
2. In one message, show the discovered folders and scan/environment findings, then ask for scope, quality mode, ASR choice, translation model, QC model, output format, and the conditional WAV-only preference. Trial, promo, DLC, EX, bonus, and other side folders must be shown rather than silently skipped. No-SE folders are ASR-source candidates, not automatic translation scope.
3. After the one reply confirms scope, resolve it with `scripts/resolve_wav_only_asr_tracks.py` and the full `audio_source_report.json`. If it finds a safe equivalent native MP3, use it; if tracks are truly WAV-only, apply the WAV preference already supplied in the same questionnaire. Do not ask a second question unless the user omitted or contradicted that preference.
9. Generate `$PROJECT_ROOT/preflight_questionnaire.md` with `scripts/render_preflight_questionnaire.py`, present it verbatim, then write the confirmed choices to `$PROJECT_ROOT/run_profile.json` with `scripts/prepare_run_profile.py`, including `--confirmation-source`, `--confirmation-text`, `--preflight-questions-presented`, `--preflight-questionnaire "$PROJECT_ROOT/preflight_questionnaire.md"`, `--confirmed-item scope --confirmed-item quality_mode --confirmed-item asr --confirmed-item translate --confirmed-item qc --confirmed-item output_format`, and either `--audio-scope-report` or `--audio-scope-summary`. Add `--confirmed-item wav_only_asr_strategy` whenever the WAV-only question applies. The preflight checker rejects a “confirmed” run that omits any of these explicit acknowledgements or changes the confirmation sheet after the user responds.
10. Sync confirmed stage model choices with `scripts/manage_model_profile.py sync-run-profile "$PROJECT_ROOT"` when a model profile should drive later commands.
11. Before each model stage, run `scripts/check_preflight.py "$PROJECT_ROOT" --stage <asr|translate|qc>`.
...
Production ASR, translation, and QC scripts require `--project-root "$PROJECT_ROOT"` and enforce the preflight gate. There is no normal-production bypass flag.
...
When the confirmed translation or QC route is `mcp`, record the selected MCP server/tool and provider/model (when known) in the run-profile notes for auditability, but invoke the registered MCP tool rather than calling `/chat/completions` directly. The agent must still complete project preflight before the call; server-side validation is only an additional guard. Do not pass a key to MCP, do not call `prepare_model_stage.py` against a fake MCP base URL, and do not silently fall back to the agent's own model. If the selected MCP tool is unavailable or reports missing configuration, stop and ask the user to repair that MCP route or choose a direct backend.
...
Model invocation discipline: when a workflow step says translation, mandatory model QC, or optional QC refinement, the agent must call either the resolved configured model endpoint through the relevant script or the explicitly confirmed MCP tool. The agent's own reasoning model is not a substitute for configured model QC, including second and later refinement rounds. The agent may orchestrate commands, read reports, compare evidence, and apply accepted corrections, but it must not silently replace `scripts/qc_srt_omlx.py`, the configured `/chat/completions` QC model, or the selected MCP QC tool with self-QC. If the configured route is unavailable, stop and report the missing service or ask for a backend change; do not "helpfully" continue by using the agent model as the QC model.
...
- The agent must give every normalized QC candidate exactly one closure decision: `accept`, `reject`, or `defer`. `accept` requires source evidence and a minimal replacement; `reject` requires a reason; `defer` requires `review_required=true` and a concrete human/audio review method. Do not leave `pending` items at the end of correction.
- High-risk `semantic`, `omission`, `hallucination`, `terminology`, and `role_tone` changes may be accepted only with script, audio, or sufficient Japanese-context evidence. Do not use a QC suggestion as source evidence. When the proposed rewrite adds unsupported people, actions, emotions, causality, degree, or other meaning, defer it to review instead of applying it.
- Generate normalized items with `scripts/review_qc_report.py`, validate their closure with `scripts/validate_qc_closure.py`, and apply only closure-approved items through `scripts/apply_qc_decisions.py --closure-report ... --apply`. The closure script writes deferred items to `qc_manual_review.md/json`; these are visible delivery risks, not silently discarded candidates.
...
- When human review is needed, use the `qc_manual_review.md/json` queue generated by `scripts/validate_qc_closure.py`; it includes the candidate, source/context, missing evidence, and review method.
...
- A registered subtitle-translation MCP is an explicit translation/QC option. Use it only after the user confirms its server/tool in preflight; the agent must retain the project preflight gate, never handle the MCP's provider credential, and require QC to return candidate reports without directly editing subtitles.
...
4. Confirm mandatory QC is completed, `qc_closure_report.json` passes, and clear issues are corrected. List every deferred manual/audio review item and its queue path in the delivery report.
