...
**Conversation gate:** after scans finish, send one blocking confirmation message before any model call. It must explicitly ask for the ASR, translation, and QC choices as three separate items; never compress them into “use the recommended models?” or infer one stage’s model from another. If new ASR is required and the selected inputs are WAV-only, the same message must also ask whether to use `mp3_cache` or `original_wav`. Do not create a confirmed profile, call a production command, or continue after a partial answer: keep the unresolved items visible and ask only for those items. A user saying “use defaults” authorizes defaults only when it clearly covers all still-unresolved choices.

**Every-task question set:** for every new subtitle task, the preflight message must cover all of the following before work starts: (1) translation scope, (2) ASR backend/base URL/model and reuse versus new ASR, (3) translation backend/base URL/model, (4) QC backend/base URL/model, and (5) WAV preparation. WAV preparation is a conditional question, but never a silent omission: when a selected track is truly WAV-only, ask `mp3_cache` versus `original_wav`; when a safe native MP3 was found, explicitly state that it will be used and that no temporary conversion is needed; when ASR is reused, explicitly state that WAV preparation is not applicable. Defaults may be proposed, but none of these five decisions may be silently assumed.
...
8. When this run needs new ASR, resolve the selected scope with `scripts/resolve_wav_only_asr_tracks.py` and the full `audio_source_report.json`. If it finds a safe equivalent native MP3 in another directory, treat it as the ASR input without asking. Only if its resulting report still contains WAV-only ASR tracks, list those tracks and confirm either `mp3_cache` or `original_wav`; the temporary MP3 option must preserve stereo and applies only to those tracks.
