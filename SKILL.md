...
9. Write the confirmed choices to `$PROJECT_ROOT/run_profile.json` with `scripts/prepare_run_profile.py`, including `--confirmation-source`, `--confirmation-text`, `--preflight-questions-presented`, `--confirmed-item scope --confirmed-item quality_mode --confirmed-item asr --confirmed-item translate --confirmed-item qc --confirmed-item output_format`, and either `--audio-scope-report` or `--audio-scope-summary`. Add `--confirmed-item wav_only_asr_strategy` whenever the WAV-only question applies. The preflight checker rejects a “confirmed” run that omits any of these explicit acknowledgements.
